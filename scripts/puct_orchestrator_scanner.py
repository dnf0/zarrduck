        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                ...
                test_res = subprocess.run(
                    ["cargo", "test", "-p", "geozarr_core", "test_read_chunk_subset"],
                    capture_output=True, text=True, cwd="geozarr_core", timeout=EVAL_TIMEOUT_SECS
                )
            ...
            bench_res = subprocess.run(
                ["cargo", "bench", "-p", "geozarr_core", "--bench", "scanner_bench"],
                capture_output=True, text=True, cwd="geozarr_core", timeout=EVAL_TIMEOUT_SECS
            )

            import re
            match = re.search(r'time:\s+\[.*?([0-9.]+)\s+(ns|µs|ms)\s+.*?\]', bench_res.stdout)
            if match:
                val = float(match.group(1))
                unit = match.group(2)
                time_us = val
                if unit == 'ns': time_us = val / 1000.0
                elif unit == 'ms': time_us = val * 1000.0
...
if __name__ == "__main__":
    orchestrator = PUCTOrchestrator(
        state_file="geozarr_core/candidates/mcts_state_scanner.json",
        target_file="geozarr_core/src/scanner.rs"
    )
