...
fn bench_scanner(c: &mut Criterion) {
    ...
    let array_write = zarrs::array::ArrayBuilder::new(
        ...
        zarrs::array::chunk_grid::ChunkGrid::new(RegularChunkGrid::new(chunk_shape.clone().try_into().unwrap())),
    ...
    array_write.store_chunk_elements(&[0, 0, 0], &chunk_data).unwrap();
    ...
    c.bench_function("scanner_read_chunk_subset_remote", |b| {
        b.iter(|| {
            let (elements, _) = reader.read_chunk_subset::<f32>(
