# Zarrduck CLI Shell Auto-Completion Design

**Date:** 2026-05-18

## 1. Context & Purpose
To maximize user-friendliness, the `zarrduck` CLI should support shell auto-completion. This allows users to press `<TAB>` in their terminal to automatically complete command names (e.g., `info`, `extract`, `shell`) and their respective arguments without having to constantly reference the `--help` menu.
...
2. Invoke `clap_complete::generate()`, passing the requested shell enum, the command representation, the binary name (`"zarrduck"`), and a mutable reference to standard output (`&mut std::io::stdout()`).
3. Exit cleanly.
...
## 6. Testing Strategy
- A new integration test using `assert_cmd` will invoke `zarrduck completions bash` and assert that the output string contains standard bash completion syntax (e.g., `_zarrduck() {`).